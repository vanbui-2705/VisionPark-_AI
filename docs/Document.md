...
| Phiên bản | 1.3.0 (Codebase Baseline) |
...
| 1.3.0 | 24/08/2026 | BA / Tech Lead | Chốt cây codebase có parent rõ ràng, tích hợp ALPR trong backend và dùng một frontend theo vai trò |
...
| --- | --- |
| Tác nhân chính | Nhân viên vận hành |
| Tác nhân phụ | Phân hệ ALPR, dịch vụ VietQR |
...
| Phân hệ ALPR | Ảnh vào; biển số, bbox, confidence trả ra | Cho phép nhập tay; lưu trạng thái lỗi |
...
- FastAPI là framework backend chính; ALPR là phân hệ độc lập về mã nguồn nhưng cùng nằm dưới parent `backend/app` trong MVP.
...
- Tách ranh giới module ALPR khỏi nghiệp vụ gửi xe để có thể thay model mà không thay đổi quy trình, đồng thời chưa phát sinh thêm service triển khai trong MVP.
